...
These KQL (Kusto Query Language) queries help you monitor your function. Run them in Azure Portal:
1. Go to your Function App → Application Insights → Logs
2. Or go to Application Insights resource directly → Logs

### All Function Invocations (Last 24 Hours)

Shows every time the function ran, whether it succeeded or failed:

```kql
requests
| where timestamp > ago(24h)
| where name == "ingest_document"
| project
    timestamp,
    success,
...
Quick overview of how many runs succeeded vs failed:
...
### Files Processed with Details

Shows which files were processed and what happened. Looks for our logging messages:

```kql
traces
| where timestamp > ago(24h)
| where message has "Processing blob:" or message has "Completed processing:"
| project
    timestamp,
    message,
    severityLevel,
...
When something goes wrong, this shows what happened:
...
### Processing Timeline (Files with Duration)

Shows each file processed with how long it took:
...
| join kind=leftouter (
    traces
    | where message has "Processing blob:"
    | extend filename = extract("Processing blob: ([^,]+)", 1, message)
) on operation_Id
| project
    timestamp,
    filename,
...
Good for spotting patterns in when files are uploaded:
...
Shows actual error messages when things fail:

```kql
exceptions
| where timestamp > ago(24h)
| where cloud_RoleName has "func-secondbrain"
| project
    timestamp,
    problemId,
    outerMessage,
...
Shows the full journey of a document through the function (parsing, chunking, etc.):
...
Track if processing is getting slower:
...
| where success == true
| summarize
    avg_duration = avg(duration)/1000,
    p95_duration = percentile(duration, 95)/1000,
    max_duration = max(duration)/1000
    by bin(timestamp, 1d)
| render timechart
```
